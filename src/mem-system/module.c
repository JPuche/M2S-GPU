...
/* Return a way that stores the block that meets the introduced policy */
int mod_find_block_in_set(struct mod_t *mod, unsigned int addr, int set,
        int *way_ptr, int *tag_ptr, int *state_ptr)
{
        struct cache_t *cache = mod->cache;
        ...
        {
        /* Addr is not needed in this policy */
        case cache_nway_evict_policy_exclusive:
        {
                mod_find_exclusive_block_in_set(mod, set, way_ptr, tag_ptr, state_ptr);
                if (*way_ptr >= 0)
                        break;
        }
	case cache_nway_evict_policy_lru:
	{
		/* There must be a lru queue */
		assert(cache->policy == cache_policy_lru);

		/* We must know if the lru block has been evicted without updating the lru pointer
 		 * It shouldnt, though */
		struct cache_block_t *lru_blk = cache->sets[set].way_tail;
		if (lru_blk)
		{
			/* Debug */
			if (lru_blk->dir_entry_selected)
				printf(" %lld Mod %s Set %d looking for candidate to evict. LRU is %x in way %d with state %s\n",
					esim_cycle(), mod->name, set, lru_blk->dir_entry_selected->tag, lru_blk->way, str_map_value(&cache_block_state_map, lru_blk->dir_entry_selected->state));			

			//PTR_ASSIGN(way_ptr, lru_blk->way);
			*(way_ptr) = lru_blk->way; // Same as PTR_ASSIGN(way_ptr, lru_blk->way);
			*tag_ptr = lru_blk->dir_entry_selected->tag;
			*state_ptr = lru_blk->dir_entry_selected->state;
		}		
		
		if (*way_ptr >= 0)
			break;
	}
        case cache_nway_evict_policy_next:
        {
		/* TODO */
		//mod_find_next_block_in_set(mod, set, way_ptr, tag_ptr, state_ptr);
                printf("No block with no sharers in the upper level found. Code next policy.\n");
                break;
        }
        case cache_nway_evict_policy_random:
        {
		/* TODO */
		//mod_find_random_block_in_set(mod, set, way_ptr, tag_ptr, state_ptr);
                break;
        }

        default:
                fatal("%s: Invalid NWay evict policy.\n", __FUNCTION__);
                break;
        }
...
/* Return the number of currently invalid blocks in the set */
int mod_count_invalid_blocks_in_set(struct mod_t *mod, int set)
{
        struct cache_t *cache = mod->cache;
        struct cache_block_t *blk;

        int way, num_invalids = 0;

        for (way = 0; way < cache->assoc; way++)
        {
                blk = &cache->sets[set].blocks[way];
                if (blk->dir_entry_selected->state == cache_block_invalid)
                        num_invalids++;
        }
...
int mod_find_exclusive_block_in_set(struct mod_t *mod, int set,
        int *way_ptr, int *tag_ptr, int *state_ptr)
{
        struct cache_t *cache = mod->cache;
        struct cache_block_t *blk;
        ...
        for (way = 0; way < cache->assoc; way++)
        {
                ...
                {
                        dir_entry = dir_entry_get(dir, set, way, z, w);

                        if (dir_entry->num_sharers == 0 && blk->dir_entry_selected->state)
                        {
                                //printf("Block %x found in mod %s set %d way %d with no sharers.\n", 
                                //       blk->tag, mod->name, set, way);
                               
                               	/* Dir entry and dir_selected_entry should always be the same in our system */
                                assert(dir_entry->state);

                                tag = blk->dir_entry_selected->tag;
                                state = blk->dir_entry_selected->state;
...
out:
        ...
        /* Block with no sharers found  */
        PTR_ASSIGN(tag_ptr, tag);
        PTR_ASSIGN(way_ptr, way);
        PTR_ASSIGN(state_ptr, state);
