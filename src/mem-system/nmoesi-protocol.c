...
void mod_handler_nmoesi_find_and_lock(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_FIND_AND_LOCK)
	{
		...
		if (stack->forced_eviction && !stack->eviction)
			mem_debug("  %lld %lld 0x%x %s find and lock for forced eviction (blocking=%d)\n",
                                esim_time, stack->id, stack->addr, target_mod->name, stack->blocking);
	...
	if (event == EV_MOD_NMOESI_FIND_AND_LOCK_PORT)
	{
		...
		if (stack->hit)
			mem_debug("    %lld 0x%x %s hit: set=%d, way=%d, state=%s\n", stack->id,
                ...
                /* If set and way are not the same than in the forced eviction, that FE is not valid and must be aborted */
		if (stack->forced_eviction && target_mod->level == 2)
		{
			if (stack->set != stack->forced_evict_set || stack->way != stack->forced_evict_way || !stack->dir_entry)
			{
				mem_debug("  %lld %lld 0x%x %s Invalid forced evict fe-set=%d, fe-way=%d, fe-state=%s vs set=%d, way=%d\n",
                                        esim_time, stack->id, stack->tag, target_mod->name, stack->forced_evict_set, stack->forced_evict_way,
                                        str_map_value(&cache_block_state_map, stack->forced_evict_state), stack->set, stack->way);
		...
		/* After Sisco's mess, we check the FE cases */
		/* Sanity checks */
		if (stack->forced_eviction && target_mod->level == 2)
		{
			assert(stack->set == stack->forced_evict_set);
			assert(stack->way == stack->forced_evict_way);
			assert(stack->hit);
		}

		/* Case 0: FE targets an already locked entry: release and sched Finish. */
		if (dir_lock->lock && stack->forced_eviction && target_mod->level == 2 && !stack->blocking)
		{
		        mem_debug("  %lld %lld 0x%x %s block already locked at set=%d, way=%d by A-%lld - aborting\n",
                               	esim_time, stack->id, stack->tag, target_mod->name, stack->set, stack->way, dir_lock->stack->id);
		...
		/* Case 1: L2 access that targets an entry already locked by a FE.
 		 * Queue the access on the dir_entry until the cleaning FE finishes its job. */
		if (dir_lock->lock && !stack->blocking && stack->miss_from_L1
			&& target_mod->cache->sets[stack->set].blocks[stack->way].cleaning && target_mod->level == 2
			&& !target_mod->cache->sets[stack->set].blocks[stack->way].waiting_for_clean)
		{
			if (!dir_entry_lock(stack->dir_entry, EV_MOD_NMOESI_FIND_AND_LOCK, stack))
			{
 	                       	mem_debug("  %lld %lld 0x%x %s block waiting for set=%d, way=%d to be clean locked by forced evict %lld\n",
	                               esim_time, stack->id, stack->tag, target_mod->name, stack->set, stack->way, dir_lock->stack->id);
                               	mod_unlock_port(target_mod, port, stack);
                               
				//int up_down_waiters = list_count(dir_lock->lock_list_up_down);
				//int down_up_waiters = list_count(dir_lock->lock_list_down_up);
				//printf("  %lld %lld 0x%x %s dir entry locked by FE %lld in set=%d way=%d has %d updown and %d downup waiters\n",
				//	esim_time, stack->id, stack->tag, target_mod->name, dir_lock->stack->id, stack->set, stack->way, up_down_waiters, down_up_waiters);
					
				stack->waiting_for_clean = 1;
				target_mod->cache->sets[stack->set].blocks[stack->way].waiting_for_clean = 1;
				return;
                        }
		...
		/* Case 2: Any other non-blocking non-missing access that targets an already locked entry 
 		 * must bounce, release the port, return error and retry. */
		...
		/* Case 3: Blocking accesses must wait in the dir_entry queue to prevent deadlocks. */ 
		if (!dir_checked)
		{
			if (!dir_entry_lock(stack->dir_entry, EV_MOD_NMOESI_FIND_AND_LOCK, stack))
			{
		...
		/* Well, this is another brick in the wall. Useless, I feel. */
		...
		*/
		...
		/* Miss */
		/* If I am not wrong, this piece of code does nothing at all, only sanity checks for Sisco's s**t. */
		/*
		if (!stack->hit)
		{
		...
		/* If a FE has successfully reached this point, this means that the block will be evicted.
 		 * Mark it as a candidate to be filled by future accesses */
		if (stack->forced_eviction && target_mod->level == 2)
		{
 	               mem_debug("  %lld 0x%x %s block forced to be evicted and marked cleaning at set=%d, way=%d\n",
                                stack->id, stack->tag, target_mod->name, stack->set, stack->way);
                                mem_debug("  %lld 0x%x %s directory locked by forced eviction at set=%d, way=%d\n",
                                stack->id, stack->tag, target_mod->name, stack->set, stack->way);

                       assert(stack->hit);
                       target_mod->cache->sets[stack->forced_evict_set].blocks[stack->forced_evict_way].cleaning = 1;
		}
	...
	if (event == EV_MOD_NMOESI_FIND_AND_LOCK_ACTION)
	{
		...
		/* This is a FE that has successfully locked the dir entry. 
 		 * Schedule de evict for this entry. */
		if (stack->hit && stack->forced_eviction && target_mod->level == 2)
		{
			stack->eviction = 1;
			...
			new_stack->set = stack->set;
			new_stack->way = stack->way;
			new_stack->tag = stack->tag;
			new_stack->return_mod = stack->target_mod; //Not sure about this
                        new_stack->dir_entry = stack->dir_entry;
			new_stack->request_dir = stack->request_dir;
			new_stack->forced_eviction = 1;
			new_stack->event = EV_MOD_NMOESI_EVICT;

			mem_debug("  %lld %lld 0x%x %s forced evict scheduling evict event in set=%d, way%d\n", esim_time, stack->id,
				stack->tag, target_mod->name, stack->set, stack->way);

			/* Debug */
			//invalid_blocks = mod_count_invalid_blocks_in_set(target_mod, stack->set);
			//printf("  %lld: Number of inflight forced evicts and invalids in mod %s set %d: %d and %d\n",
                        //       esim_cycle(), target_mod->name, stack->set, target_mod->cache->sets[stack->set].inflight_evs, invalid_blocks);	
		...
		else if (stack->forced_eviction)
		{
			...
			/* Debug */
			mem_debug("  %lld %lld 0x%x %s forced evict finished probably because it reached MM\n", esim_time, stack->id,
				stack->tag, target_mod->name);
			//printf("  %lld: Forced Ev that is either a miss or an access to a level != 2. NMOESI.c:2361\n", esim_cycle());
			return;
		}

		/* Well now here is where Forced Evictions are meant to be issued */
		int invalid_blocks = mod_count_invalid_blocks_in_set(target_mod, stack->set);
		int inflight_evs = target_mod->cache->sets[stack->set].inflight_evs;	

				/* Even hits can issue forced evs */
		if ((inflight_evs + invalid_blocks) < target_mod->cache->max_nway_evicts && target_mod->level == 2)
		{
			struct mod_stack_t *ev_stack;
			int tag, way, set_found, state, i;

			//if (target_mod->level == 2)
			//	printf("  %lld %lld: F&L ACTION: mod %s set %d way %d\n\tInvalid Blocks: %d\tInflight Evs: %d\tMax NWay Evs: %d\tTargetMod Level: %d\n", esim_time, stack->id, target_mod->name, stack->set, stack->way, invalid_blocks, inflight_evs, target_mod->cache->max_nway_evicts, target_mod->level);

	
			/* Issue N forced evs, being N */
			for (i = inflight_evs + invalid_blocks; i < target_mod->cache->max_nway_evicts; i++)
			{
				/* Find a block to be evicted. 
				 * Design suitable eviction policies here. */
				set_found = mod_find_block_in_set(target_mod, stack->tag, stack->set, &way, &tag, &state);
				
				/* Friendly reminder: Set is the only common thing in stack and ev_stack in forced evs. */
				if (set_found <= target_mod->cache->assoc && set_found >= 0)
				{
					mod_stack_id++;
					ev_stack = mod_stack_create(mod_stack_id, target_mod, tag, ESIM_EV_NONE, NULL);
					ev_stack->request_dir = mod_request_up_down;
					ev_stack->set = stack->set;
					ev_stack->forced_eviction = 1;
					ev_stack->forced_evict_set = stack->set;
					ev_stack->forced_evict_way = way;
					ev_stack->forced_evict_state = state;
					ev_stack->way = stack->way;			/* Way and FE-Way should always be the same  */
					ev_stack->tag = tag;
					ev_stack->addr = tag;
					ev_stack->event = EV_MOD_NMOESI_FIND_AND_LOCK;	/* Sisco's way of life */

					/* Increment the number of in-flight evicts for this set */
					target_mod->cache->sets[stack->set].inflight_evs++;

					/* Schedule a segmented eviction for this block */
					esim_schedule_mod_stack_event(ev_stack, 0);
				}
			}
				 //printf("  %lld: Number of inflight forced evicts in mod %s set %d: %d\n",
	                         //        esim_cycle(), target_mod->name, stack->set, target_mod->cache->sets[stack->set].inflight_evs);
		}
		...
		if (!stack->uncacheable && !stack->hit && stack->dir_entry->state)
		{
			...
			/* Either one or N evicts have been scheduled. 
			 * Return and wait until the evict comes back to F&L Finish. */
			return;
		}
	...
	if (event == EV_MOD_NMOESI_FIND_AND_LOCK_FINISH)
	{
		...
		if (stack->err && !stack->forced_eviction)
		{
			if (stack->mshr_locked != 0)
			{
				mshr_unlock(target_mod, stack);
		...
		/* Forced Evs are not retried and do not lock the mshr */
		if (stack->err && stack->forced_eviction)
		{
			assert(stack->eviction);
			assert(target_mod->level == 2);

                        mem_debug("  %lld 0x%x %s forced eviction aborted due to a mm locked entry at set=%d, way=%d\n",
                                stack->id, stack->tag, target_mod->name, stack->forced_evict_set, stack->forced_evict_way);

			if (stack->dir_lock)
				dir_entry_unlock(stack->dir_entry);

                        target_mod->cache->sets[stack->forced_evict_set].inflight_evs--;
                        target_mod->cache->sets[stack->forced_evict_set].blocks[stack->forced_evict_way].cleaning = 0;
		...
		if (stack->eviction)
		{
			...
			if (stack->forced_eviction && target_mod->level == 2)
			{
				...
				if (stack->dir_lock)
                                ...
                                mem_debug("  %lld 0x%x %s directory unlocked by forced eviction at set=%d, way=%d\n",
                                        stack->id, stack->tag, target_mod->name, stack->set, stack->way);
				add_forced_eviction(target_mod->level);
                                //mod->forced_evictions++;				
			}

		}

		/* Another FE faulting case. Debug and update counters. */
		if (!stack->eviction && stack->forced_eviction && target_mod->level == 2)
		{
                        mem_debug("  %lld 0x%x %s forced eviction aborted at set=%d, way=%d\n",
                                        stack->id, stack->tag, target_mod->name, stack->set, stack->way);
...
void mod_handler_nmoesi_evict(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_EVICT)
	{
		...
		new_stack->forced_eviction = stack->forced_eviction;
	...
	if (event == EV_MOD_NMOESI_EVICT_PROCESS)
	{
		...
		else if (stack->reply == reply_ack_data)
		{
			if(target_mod->kind == mod_kind_main_memory &&
				...
				if(stack->wavefront == NULL || stack->forced_eviction){
...
void mod_handler_nmoesi_read_request(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_READ_REQUEST_RECEIVE)
	{
		...
		*/
		...
		stack->event = EV_MOD_NMOESI_FIND_AND_LOCK;
		stack->miss_from_L1 = 1;
		//stack->miss_from_L1 |= (1 << (target_mod->level - 1))
	...
	if (event == EV_MOD_NMOESI_READ_REQUEST_ACTION)
	{
		...
		if (target_mod->level == 2 && stack->request_dir == mod_request_up_down && !stack->forced_eviction && stack->client_info)
		{
			long long penalty_cycles;
			stack->l2_leave_cycle = stack->client_info->arch->timing->cycle;

			penalty_cycles = stack->l2_leave_cycle - stack->l2_access_cycle;
			
			target_mod->cache->sets[stack->set].penalty_accesses++;
			target_mod->cache->sets[stack->set].penalty_latency += penalty_cycles;

			target_mod->penalty_cycles += penalty_cycles;
		}
	...
	if (event == EV_MOD_NMOESI_READ_REQUEST_UPDOWN_MISS)
	{
		...
		//	stack->shared ? cache_block_shared : cache_block_exclusive);
			...
			/* Waiting for clean  */
			if (stack->waiting_for_clean && target_mod->cache->sets[stack->set].blocks[stack->way].waiting_for_clean
				&& target_mod->level == 2)
			{
				stack->waiting_for_clean = 0;
				target_mod->cache->sets[stack->set].blocks[stack->way].waiting_for_clean = 0;
			}
                }
...
//		    new_stack->forced_eviction = stack->forced_eviction;
	...
	if (event == EV_MOD_NMOESI_WRITE_REQUEST_UPDOWN_FINISH)
	{
		 ...
		 * Set mod as sharer and owner. */
		    ...
		    /* Stack waiting for clean */
		    /* Reset the flags of F.E. for this line. */
		    if (stack->waiting_for_clean && target_mod->cache->sets[stack->set].blocks[stack->way].waiting_for_clean
			    && target_mod->level == 2)
		    {
		            stack->waiting_for_clean = 0;
