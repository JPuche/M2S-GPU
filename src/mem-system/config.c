...
static void mem_config_read_general(struct config_t *config)
{
	...
	/* Mshr Protocol */
	mshr_protocol_str = config_read_string(config, section, "MshrProtocol", "default");

	if(!strncasecmp(mshr_protocol_str,"default", 8))
	{
	...
	char *section)
{
	...
	char *evict_policy_str;
	enum cache_nway_evict_policy_t evict_policy;
	...
	policy_str = config_read_string(config, buf, "Policy", "LRU");
        evict_policy_str = config_read_string(config, buf, "EvictPolicy", "Exclusive");
	mshr_size = config_read_int(config, buf, "MSHR", 16);
	num_ports = config_read_int(config, buf, "Ports", 2);
	max_nway_evicts = config_read_int(config, buf, "NWayEvicts", 0);
	...
	evict_policy = str_map_string_case(&cache_nway_evict_policy_map, evict_policy_str);
	if (evict_policy == cache_nway_evict_policy_invalid)
                fatal("%s: cache %s: %s: invalid block nway evict policy. \n%s",
                        mem_config_file_name, mod_name,
                        evict_policy_str, mem_err_config_note);
	if (num_sets < 1 || (num_sets & (num_sets - 1)))
		fatal("%s: cache %s: number of sets must be a power of two and "
	...
	/* Create cache */
        ...
        mod->cache->mod = mod;
	mod->cache->max_nway_evicts = max_nway_evicts;
	mod->cache->evict_policy = evict_policy;
	...
	char *section)
{
	...
	mod->cache = cache_create(mod->name, dir_size / dir_assoc, block_size,
		dir_assoc, policy, dir_entry_per_block, extra_dir_structure_type);
        mod->cache->mod = mod;
