...
struct str_map_t cache_nway_evict_policy_map =
{
        5, {
                { "Invalid", cache_nway_evict_policy_invalid },
                { "Exclusive", cache_nway_evict_policy_exclusive },
		{ "LRU", cache_nway_evict_policy_lru },
                { "Next", cache_nway_evict_policy_next },
                { "Random", cache_nway_evict_policy_random }
        }
};
...
void cache_set_block_new(struct cache_t *cache, struct mod_stack_t *stack, int state)
{
	...
	cache->sets[set].blocks[way].valid_mask = 0;
        ...
        {
						if(!stack->dir_entry->is_extra)
						{
								add_frc_access(stack->target_mod->level);
						}
            if(cache->sets[set].blocks[way].dir_entry_selected->state != cache_block_invalid)
            {
                add_frc_swap(stack->target_mod->level);
...
int cache_replace_block(struct cache_t *cache, int set)
{
	...
	/* If NWay mechanism is working, try to find a line being cleaned by a forced eviction */
	/* AND, this is important, the line must also not have any previous stack waiting for it too */
	if (cache->max_nway_evicts > 0)
		for (int way = 0; way < cache->assoc; way++)
			if (cache->sets[set].blocks[way].cleaning && !cache->sets[set].blocks[way].waiting_for_clean)
				/* Update LRU here maybe?? */
				return way;
