...
static void m2s_read_command_line(int *argc_ptr, char **argv)
{
	...
	for (argi = 1; argi < argc; argi++)
	{
		...
		if(!strcmp(argv[argi],"--extra-reports"))
		{
			...
			/* Read extra reports path */
			strcpy(a,argv[argi]);

			/* Copy it to global var fran_extra_reports_path */
			fran_extra_reports_path = xstrdup(a);

			/* And now let xisco do his stuff */
			strcat(a, "_ipc");
